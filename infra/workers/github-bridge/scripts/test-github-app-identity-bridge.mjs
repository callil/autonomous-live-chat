...
globalThis.fetch = async (input, init = {}) => {
	...
	if (url.endsWith("/pulls/55")) return Response.json({ state: "open", merged: false, mergeable_state: "dirty", head: { sha: "a".repeat(40) } });
	...
	if (url.endsWith("/actions/runs/902")) return Response.json({ status: "completed", conclusion: "success", html_url: "https://github.com/callil/autonomous-live-chat/actions/runs/902", display_title: "App Harness promotion · dispatch-work-42" });
	if (url.endsWith("/actions/runs/903")) return Response.json({ status: "completed", conclusion: "success", html_url: "https://github.com/callil/autonomous-live-chat/actions/runs/903", display_title: "Promote App Harness OS candidate" });
...
try {
	...
	assert.deepEqual(await capability.observeCandidatePullRequest({ number: 55 }), { number: 55, state: "open", merged: false, mergeableState: "dirty", mergeCommitSha: null, headSha: "a".repeat(40) }, "the merge-watch recovery path can observe a conflicted candidate's live merge state and its live head");
	assert.deepEqual(await capability.observeCandidatePullRequest({ number: 56 }), { number: 56, state: "open", merged: false, mergeableState: "unknown", mergeCommitSha: null, headSha: null }, "a lazily computed mergeability projects as unknown, never as evidence");
	assert.deepEqual(await capability.observeCandidatePullRequest({ number: 57 }), { number: 57, state: "closed", merged: true, mergeableState: "unknown", mergeCommitSha: "c".repeat(40), headSha: null }, "a merged candidate's observation carries the merge commit, so a lost merged webhook is reconcilable");
