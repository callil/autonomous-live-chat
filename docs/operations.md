...
- **Rebuilt platform (phase 3).** `provision-runtime-secrets.yml` target `platform` installs, on Worker `app-harness-platform`: `ADMIN_TOKEN` (owner levers + the session-signing key derivation) from `APP_HARNESS_ADMIN_TOKEN`; `GITHUB_APP_ID`/`GITHUB_APP_INSTALLATION_ID`/`GITHUB_APP_PRIVATE_KEY` from the same `APP_HARNESS_*` sources the legacy bridge uses; `GITHUB_WEBHOOK_SECRET` from `APP_HARNESS_PLATFORM_WEBHOOK_SECRET` (a **repository webhook** pointing at `POST /api/hooks/poke` on the platform — distinct from the GitHub App webhook the legacy bridge owns); and, when `APP_HARNESS_OPENAI_API_KEY` is set, `OPENAI_API_KEY` for the Doctor (the same repo-level secret the platform runner rides; absent, the Doctor degrades to the deterministic park-for-human stub). Target `platform-runner` installs `OPENAI_API_KEY` on Worker `app-harness-platform-runner` from `APP_HARNESS_OPENAI_API_KEY`. The product Worker (`app-harness-product`) holds no secrets at all.
