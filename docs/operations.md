...
- `deploy-platform.yml` deploys the platform runner and then the platform Worker on every push to main touching `platform/**`. Agent branches can never reach this path — the platform firewall fails any `room/*` diff touching frozen surfaces.
...
Owner levers, all requiring the ADMIN_TOKEN bearer:
