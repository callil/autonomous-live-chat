...
3. The deployed Cloudflare OS starter customization is not fully versioned in this repository. Its implementation should eventually live under `infra` or in a clearly linked dedicated repository.
4. The framework-free demo and React package share a documented target contract but not one imported implementation. A small framework-neutral core package would remove that duplication when a second production host exists.
5. End-to-end acceptance still depends on healthy OpenAI, GitHub Actions, GitHub, and Cloudflare services. Durable recovery handles interruption, but cannot manufacture a successful external result during an outage or exhausted API balance.
6. `npm audit` currently reports transitive `undici` advisories through the latest Wrangler/Miniflare release. Miniflare pins that version exactly; forcing a dependency override or downgrading Wrangler was not accepted without upstream compatibility evidence. Track the next Wrangler release and re-run the audit.
