...
Each stack has one root issue, one base SHA, a bounded lane, a monotonic generation, and an ordered list of intent slices. The first branch is based on the recorded `main` SHA. Every later branch is based on its immediate parent branch, and each GitHub pull request uses that parent as its base. A small request uses the same ledger as a one-node stack fast path; dependent slices add ordered child PRs.
...
- If `main` advances before a one-node candidate exists, the coordinator advances that root to a new generation automatically. If the candidate already exists, its head remains immutable and CI revalidates it against the newly observed `main` SHA.
