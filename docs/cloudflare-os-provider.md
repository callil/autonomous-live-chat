...
The isolated runner is deployed at `app-harness-os-native-git` and has passed remote Cloudflare Sandbox probes plus a repository-scoped native checkout. App Harness has private service bindings to a GPT-5.6 Terra planning Worker and the runner. A bounded text request first creates an `cloudflare-os-planning` GitHub issue, then reaches the planner; only a schema-validated accent-color plan is forwarded to the runner. The Durable Object persists the plan and model provenance, then uses its single at-least-once alarm to start or resume the runner job. The runner receives the plan, stack generation, issue number, and allowed repository—not raw user prose or a model response.
