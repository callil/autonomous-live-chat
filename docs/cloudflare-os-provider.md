...
After checkout, the runner—not the planner—runs a bounded Responses function-tool loop. The model first lists and reads the real allowed docs, then may apply a unified patch, inspect the diff, run the fixed checks, and finish. It has no shell or network tool. The runner refuses a write before the relevant file was read, lets the model inspect and retry a failed patch within a small round limit, runs `git diff --check`, proves the changed path list stays in the documentation allowlist, then stages only `README.md` and `docs/`, commits, pushes, and opens a PR. A malformed patch, a non-documentation path, an empty diff, an exhausted tool loop, or a failed check terminates the candidate without a PR claim.
