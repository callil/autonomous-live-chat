...
const job = createOsNativeGitJob({ manifest, plan: { kind: "repository-task", request: "Clarify the project intent in README." } });
const pinnedJob = createOsNativeGitJob({ manifest, plan: { kind: "repository-task", request: "Clarify the project intent in README." }, parentBaseSha: "a".repeat(40) });
...
assert.throws(() => createOsNativeGitJob({ manifest, plan: { kind: "repository-task", request: "" } }));
...
assert.throws(() => createOsNativeGitJob({ manifest, plan: { kind: "repository-task", request: "Clarify README." }, parentBaseSha: "main" }));
