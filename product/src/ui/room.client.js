 ...
 * Plain chat is chat; only explicit Target / Comment / Draw envelopes request
 * changes through the same-origin platform room protocol.
 */
(() => {
	"use strict";
	...
	const activeSessionCount = $("#active-session-count"), queueStatusDot = $("#queue-status-dot");
	...
	let queueCount = 0, optimisticRequests = 0;
	...
	function updateActiveSessions() {
		const count = queueCount + pending.size + optimisticRequests;
		activeSessionCount.textContent = String(count);
		queueStatusDot.classList.toggle("is-busy", count > 0);
	...
	function provenanceLinks(refs) {
		const wrap = document.createElement("span"); wrap.className = "provenance";
		if (Number.isInteger(refs.prNumber)) {
			const pr = document.createElement("a");
			pr.href = `${REPO_URL}/pull/${refs.prNumber}`; pr.target = "_blank"; pr.rel = "noreferrer"; pr.textContent = `PR #${refs.prNumber}`; wrap.append(pr);
		}
		if (typeof refs.sha === "string" && refs.sha.length >= 7) {
			const commit = document.createElement("a");
			commit.href = `${REPO_URL}/commit/${refs.sha}`; commit.target = "_blank"; commit.rel = "noreferrer"; commit.textContent = refs.sha.slice(0, 7); wrap.append(commit);
		}
	...
	function addFeedItems(items) {
		for (const item of items || []) {
			...
			row.className = `kind-${item.kind}`; row.dataset.seq = String(item.seq);
			const text = document.createElement("span"); text.textContent = item.text; row.append(text);
	...
	function renderQueue(chips) {
		...
		for (const chip of chips || []) {
			...
			el.className = "queue-chip"; el.textContent = chip.label; queueChips.append(el);
		}
		queueCount = (chips || []).length;
	...
	function renderPending() {
		...
		for (const [intentId, ack] of pending) {
			...
			const row = stampDynamic(document.createElement("div"), "pending-ack"); row.className = "pending-ack";
			const seconds = Math.max(0, Math.ceil((ack.cancelDeadline - now) / 1000));
			const label = document.createElement("span"); label.textContent = `Request accepted — dispatching in ${seconds}s`;
			const cancel = document.createElement("button"); cancel.type = "button"; cancel.textContent = "Cancel";
			cancel.addEventListener("click", () => { send({ type: "request:cancel", intentId }); pending.delete(intentId); renderPending(); });
			row.append(label, cancel); pendingAcks.append(row);
		}
		updateActiveSessions();
	...
	function captureEnvelope(element) {
		const rect = element.getBoundingClientRect(), computed = getComputedStyle(element), computedStyles = {};
		...
		return {
			...
			selector: `[data-loc="${element.dataset.loc}"]`, tag: element.tagName.toLowerCase(), page: location.pathname,
			rect: { x: Math.round(rect.x), y: Math.round(rect.y), width: Math.round(rect.width), height: Math.round(rect.height) },
			computedStyles, viewport: { width: innerWidth, height: innerHeight },
		};
	}
	...
	function setMode(next) {
		...
		document.body.classList.toggle("mode-target", mode === "target"); document.body.classList.toggle("mode-comment", mode === "comment"); document.body.classList.toggle("mode-draw", mode === "draw");
		if (mode === null) clearSelection();
		if (mode !== null) closeRequestComposer();
	}
	function clearSelection() {
		hovered?.classList.remove("loc-hover"); selected?.classList.remove("loc-selected");
		hovered = selected = selectedEnvelope = pendingDraw = null;
	}
	function positionRequestComposer(anchor) {
		requestComposer.style.left = "0px"; requestComposer.style.top = "0px";
		const rect = requestComposer.getBoundingClientRect();
		let left = anchor.x + COMPOSER_GUTTER, top = anchor.y + COMPOSER_GUTTER;
		...
		requestComposer.style.left = `${left}px`; requestComposer.style.top = `${top}px`;
	}
	function openRequestComposer(kind, contextLabel, anchor) {
		requestComposer.dataset.kind = kind; requestContext.textContent = contextLabel; requestInput.value = "";
		requestInput.placeholder = kind === "comment" ? "Leave feedback…" : "Request a change…";
		requestError.textContent = ""; requestError.classList.remove("ok"); requestSubmit.disabled = false; requestComposer.hidden = false;
		positionRequestComposer(anchor); requestInput.focus();
	}
	function closeRequestComposer() { requestComposer.hidden = true; clearSelection(); }

	document.addEventListener("pointerover", (event) => {
		if (mode !== "target" && mode !== "comment") return;
		const candidate = locFor(event.target); if (candidate === hovered) return;
		hovered?.classList.remove("loc-hover"); hovered = candidate; hovered?.classList.add("loc-hover");
	}, true);
	document.addEventListener("click", (event) => {
		if (mode !== "target" && mode !== "comment") return;
		const candidate = locFor(event.target); if (!candidate || candidate.closest(".overlay-frame")) return;
		event.preventDefault(); event.stopImmediatePropagation(); selected?.classList.remove("loc-selected"); selected = candidate; selected.classList.add("loc-selected");
		selectedEnvelope = captureEnvelope(candidate);
		const kind = mode === "comment" ? "comment" : "target"; setModeOff();
		openRequestComposer(kind, `${kind === "comment" ? "Comment on" : "Target"}: ${selectedEnvelope.dataLoc}`, { x: event.clientX, y: event.clientY });
	}, true);
	function setModeOff() {
		mode = null; for (const button of [toolTarget, toolComment, toolDraw]) button.setAttribute("aria-pressed", "false");
	...
	drawLayer.addEventListener("pointerdown", (event) => {
		if (mode !== "draw") return; event.preventDefault(); drawing = true;
		draftPoints = [{ x: Math.round(event.clientX), y: Math.round(event.clientY) }];
		draftPath = document.createElementNS("http://www.w3.org/2000/svg", "path"); drawLayer.append(draftPath); drawLayer.setPointerCapture(event.pointerId);
	});
	drawLayer.addEventListener("pointermove", (event) => {
		if (!drawing) return;
		const point = { x: Math.round(event.clientX), y: Math.round(event.clientY) }, previous = draftPoints[draftPoints.length - 1];
		if (Math.hypot(point.x - previous.x, point.y - previous.y) < MIN_DRAW_DISTANCE) return;
		draftPoints.push(point); draftPath.setAttribute("d", pathFor(draftPoints));
	});
	function finishDraw(event) {
		if (!drawing) return; drawing = false; drawLayer.releasePointerCapture?.(event.pointerId);
		...
		pendingDraw = { points: draftPoints.slice(0, 600), envelope: captureEnvelope(under), path: draftPath }; draftPath = null; setModeOff();
		openRequestComposer("draw", `Drawing over: ${pendingDraw.envelope.dataLoc}`, draftPoints[draftPoints.length - 1]);
	}
	drawLayer.addEventListener("pointerup", finishDraw); drawLayer.addEventListener("pointercancel", finishDraw);

	requestComposer.addEventListener("submit", (event) => {
		event.preventDefault();
		const kind = requestComposer.dataset.kind, text = requestInput.value.trim();
		...
		const clientSubmissionId = crypto.randomUUID(); let annotation;
		if (kind === "draw") {
		...
		optimisticRequests += 1; updateActiveSessions();
		requestSubmit.disabled = true; requestError.textContent = "Submitting…";
	});
	requestInput.addEventListener("keydown", (event) => {
		if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); if (!requestSubmit.disabled) requestComposer.requestSubmit(); }
	});
	requestCancel.addEventListener("click", () => { pendingDraw?.path?.remove(); closeRequestComposer(); });
	document.addEventListener("keydown", (event) => {
		if (event.key === "Escape") { if (!requestComposer.hidden) { pendingDraw?.path?.remove(); closeRequestComposer(); } setModeOff(); }
	});
	toolTarget.addEventListener("click", () => setMode("target")); toolComment.addEventListener("click", () => setMode("comment")); toolDraw.addEventListener("click", () => setMode("draw"));
	...
	function connect() {
		...
		socket.addEventListener("message", ({ data }) => {
			...
			if (event.type === "room:snapshot") { (event.chat || []).forEach(addChat); if (event.feed) { addFeedItems(event.feed.items); renderQueue(event.feed.queue); } if (event.you) identity = event.you; }
			...
			if (event.type === "request:ack") {
				optimisticRequests = Math.max(0, optimisticRequests - 1); pending.set(event.intentId, { cancelDeadline: event.cancelDeadline }); renderPending();
				pendingDraw?.path?.remove(); requestError.classList.add("ok"); requestError.textContent = "Accepted — watch the feed. You can cancel for a few seconds.";
				requestSubmit.disabled = false; setTimeout(closeRequestComposer, 1600);
			}
			if (event.type === "room:notice") {
				if (!requestComposer.hidden) {
					if (requestSubmit.disabled) { optimisticRequests = Math.max(0, optimisticRequests - 1); updateActiveSessions(); }
					requestError.classList.remove("ok"); requestError.textContent = event.text; requestSubmit.disabled = false;
	...
	joinForm.addEventListener("submit", async (event) => {
		event.preventDefault(); const name = joinName.value.trim(), joinButton = joinForm.querySelector("button[type=submit]");
		...
		try {
			...
			identity = await response.json(); joinError.textContent = ""; join.hidden = true; connect();
		} catch { joinError.textContent = "Could not reach the room. Try again."; }
		finally { joinButton.disabled = false; }
	});
	fetch("/api/session").then(async (response) => { if (response.ok) { identity = await response.json(); connect(); } else join.hidden = false; }).catch(() => { join.hidden = false; });
