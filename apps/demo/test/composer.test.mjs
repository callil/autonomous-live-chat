...
assert.match(worker, /const OPERATOR_CHAT_KEY = "ledger-operator-v3"/u, "the persistent operator chat is bound to the exact App Harness capability schema");
...
assert.match(worker, /progress one step only: claim -> classification -> issue -> plan -> implementation -> candidate -> validating -> promotion -> deployed -> completed/iu, "the operator wake begins with the compact legal progression");
...
assert.match(worker, /const OPERATOR_GADGET_KEY = "app-harness-operator-v3"/u, "the persistent operator workspace cannot retain an experimental APP_HARNESS binding");
